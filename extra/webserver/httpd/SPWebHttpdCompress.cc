...
static apr_status_t process_chunk(brotli_ctx_t *ctx, const void *data, apr_size_t len, ap_filter_t *f) {
	...
	while (avail_in > 0) {
		...
		if (!BrotliEncoderCompressStream(ctx->state, BROTLI_OPERATION_PROCESS,
			...
			log::error("Brotli", "Error while compressing data");
			return APR_EGENERAL;
		}
...
static apr_status_t compress_filter(ap_filter_t *f, apr_bucket_brigade *bb) {
	request_rec *r = f->r;
	Request rctx(HttpdRequestController::get(r));

	brotli_ctx_t *ctx = (brotli_ctx_t *)f->ctx;
	apr_status_t rv;
	auto *conf = rctx.host().getCompressionConfig();
	...
	if (!ctx) {
		...
		auto encoding = StringView(r->content_encoding);
		if (!encoding.empty()) {
			ap_remove_output_filter(f);
			return ap_pass_brigade(f->next, bb);
		}
		...
		auto accepts = rctx.getRequestHeader("Accept-Encoding");
		if (accepts.empty()) {
			ap_remove_output_filter(f);
			return ap_pass_brigade(f->next, bb);
		}
		...
		if (canBeAccepted) {
			canBeAccepted = false;
			StringView ctView(rctx.getResponseHeader("Content-Type"));
		...
		rctx.setResponseHeader("Content-Encoding", "br");
		rctx.setContentEncoding("br");

		rctx.setResponseHeader("Content-Length", StringView());
		rctx.setResponseHeader("Content-MD5", StringView());

		switch (conf->etag_mode) {
		case EtagMode::AddSuffix: {
            auto etag = rctx.getResponseHeader("ETag");
            if (!etag.empty()) {
            	StringView etagView(etag); etagView.trimChars<StringView::Chars<'"'>>();
            	rctx.setResponseHeader("ETag", toString('"', etag, "-br\""));
            }
			break;
		}
		case EtagMode::Remove:
			rctx.setResponseHeader("ETag", StringView());
			break;
		case EtagMode::NoChange:
			break;
		}
